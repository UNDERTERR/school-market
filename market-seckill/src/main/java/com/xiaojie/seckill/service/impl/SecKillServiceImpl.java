...
import com.xiaojie.seckill.feign.CouponFeignService;
import com.xiaojie.seckill.feign.ProductFeignService;
...
import com.xiaojie.seckill.vo.SeckillSessionWithSkusVo;
import com.xiaojie.seckill.vo.SkuInfoVo;
    ...
    private void saveSecKillSession(List<SeckillSessionWithSkusVo> sessions) {
        sessions.stream().forEach(session -> {
            ...
            if (!redisTemplate.hasKey(key)) {
                List<String> values = session.getRelations().stream().map(sku ->
                        sku.getPromotionSessionId() + "-" + sku.getSkuId()).collect(Collectors.toList());
